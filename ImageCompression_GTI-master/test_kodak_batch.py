...
def test(args, im_dir, checkpoint_dir, CONTEXT=True, crop=None):
    ...
    image_comp = torch.load(checkpoint_dir)
...
if __name__ == "__main__":
    ...
    checkpoints= glob('./checkpoints/ae_24_*')
    dirs = glob('/kodak/kodim*.png')
    with torch.no_grad():
        for checkpoint in checkpoints:
            bpps, psnrs = 0., 0.
            for dir in dirs:
                bpp,psnr = test(args, dir, checkpoint, CONTEXT=False, crop=None)
                bpps += bpp
                psnrs += psnr
            print(checkpoint,"bpps:%0.4f, psnr:%0.4f"%(bpps/24.0, psnrs/24.0))
