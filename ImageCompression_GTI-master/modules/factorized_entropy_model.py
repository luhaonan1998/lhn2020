...
class Entropy_bottleneck(nn.Module):
    ...
    def forward(self, x, training):
        ...
        if training:
            x_likeli = self.add_noise(x)
            x = ops.Round_STE.apply(x)
        else:
            x_likeli = torch.round(x)
            x = x_likeli

        lower = self._logits_cumulative(x_likeli - 0.5, stop_gradient=False)
        upper = self._logits_cumulative(x_likeli + 0.5, stop_gradient=False)
